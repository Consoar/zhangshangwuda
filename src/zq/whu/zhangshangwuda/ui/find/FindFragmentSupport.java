...
//////////////////////////////////////////////////////////////////////////////////
	...
	{
		rootView = MyApplication.getLayoutInflater().inflate(R.layout.find_fragment_light, container, false);
		return rootView;
	}
	...
	private void init()
	{
		find_button_setting = (Button)rootView.findViewById(R.id.find_button_setting);
		find_button_help = (Button)rootView.findViewById(R.id.find_button_help);
		find_button_feedback = (Button)rootView.findViewById(R.id.find_button_feedback);
		find_button_about = (Button)rootView.findViewById(R.id.find_button_about);
		...
		if (getmTheme() == R.style.MyLightTheme)
		{
			...
			find_button_feedback.setBackgroundResource(R.drawable.ringer_button);
			find_button_about.setBackgroundResource(R.drawable.ringer_button);
		}
		else
		{
			find_button_setting.setBackgroundResource(R.drawable.ringer_dark_button);
			find_button_help.setBackgroundResource(R.drawable.ringer_dark_button);
			find_button_feedback.setBackgroundResource(R.drawable.ringer_dark_button);
			find_button_about.setBackgroundResource(R.drawable.ringer_dark_button);
		}
		
		mFindLinearLayout = (FindLinearLayout)rootView.findViewById(R.id.findlinearlayout);
		addContents();
	 ...
	 * 
	 * @param title 按钮上的文字
	 * @param extra 传递到另外一个activity的参数
	 * @param iconId_l 白版的iconID
	 * @param iconId_d 黑版的iconID
	 */
	private void addContent(String title, String extra, int iconId_l, int iconId_d)
	{
		if (getmTheme() == R.style.MyLightTheme)
			mFindLinearLayout.addButton(getActivity(), title, extra, iconId_l, THEME_LIGHT);
		else
			mFindLinearLayout.addButton(getActivity(), title, extra, iconId_d, THEME_DARK);
	}
